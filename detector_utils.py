...
PATH_TO_CKPT = TRAINED_MODEL_DIR + '/frozen_inference_graph__.pb'
# List of the strings that is used to add correct label for each box.
PATH_TO_LABELS = 'D:\\Data_Science\\iNeuron\\shredder-machine-hosur\\frozen_graphs\\vehicle.pbtxt'
...
def draw_box_on_image(num_hands_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np):
    ...
    for i in range(num_hands_detect):
        ...
        if (scores[i] > score_thresh):
            ...
            if dist:
                hand_cnt=hand_cnt+1           
            cv2.rectangle(image_np, p1, p2, color , 2, 2)
            

            #cv2.putText(image_np, 'hand '+str(i)+': '+id, (int(left), int(top)-5),
                 #       cv2.FONT_HERSHEY_SIMPLEX, 0.5 , color, 2)
            cv2.putText(image_np, 'Car '+str(i)+': ', (int(left), int(top)-5),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5 , color, 2)
